...
class ComicReaderDynamicLoadingTest {
    ...
    @Test
    fun zoomAndVerticalPanKeepsContentAtTheMovedFingerCentroid() {
        val images = (1..5).map(::imageItem)
        var successfulLoads = 0
        rule.setContent {
            var transform by remember {
        ...
        val readerHeight = rule.onNodeWithTag("comic_reader")
            .fetchSemanticsNode().boundsInRoot.height
        val expectedTargetY = readerHeight * 0.2f + 120f

        zoomComicWithVerticalPan(120f)
        rule.waitForIdle()

        val semantics = rule.onNodeWithTag("comic_reader")
            .fetchSemanticsNode().config
        val targetY = semantics[
            ComicViewportAnchorTargetYSemanticsKey
        ]
        val anchorError = semantics[
            ComicViewportAnchorErrorSemanticsKey
        ]
        assertEquals(expectedTargetY, targetY, 1f)
        assertTrue(
            "anchorError=$anchorError targetY=$targetY",
            abs(anchorError) <= 3f,
        )
    }
    ...
    private fun zoomComicWithVerticalPan(verticalPanPx: Float) {
        rule.onNodeWithTag("comic_reader")
            .performTouchInput {
                val centroid = Offset(
                    x = center.x * 1.44f,
