...
- 测试提交：`2abe296e45c7db5d387474b05ca60b48c7e439ce`
...
- 结果：**PASS**
- 证据：Gradle `BUILD SUCCESSFUL in 1m 21s`，共 `116 actionable tasks: 65 executed, 51 up-to-date`。
...
- Connected Android 测试：**NOT RUN：`adb devices -l` 没有可用模拟器或设备。**
- 暂停后播放，画面和声音继续推进：**NOT RUN：没有可用设备。**
- 暂停后拖动并释放，保持暂停且显示目标帧：**NOT RUN：没有可用设备。**
- 上述拖动后播放，画面和声音从目标位置开始：**NOT RUN：没有可用设备。**
- 连续五次暂停、拖动、播放且无冻结帧：**NOT RUN：没有可用设备。**
- 视频进入后台至少 15 秒，声音继续，返回后画面更新：**NOT RUN：没有可用设备。**
- 普通模式竖向音量弹层、静音/取消静音及硬件音量键刷新：**NOT RUN：没有可用设备。**
- 全屏右侧音量轨、左侧亮度轨及锁定模式拦截：**NOT RUN：没有可用设备。**
- 队列手动加入、整行拖动、删除普通/当前项及重启恢复：**NOT RUN：没有可用设备。**
- 进度条下方无第二加载条，中央缓冲图标在播放推进后消失：**NOT RUN：没有可用设备。**
...
- SHA-256：`5ccd5801895c143b35afd1ab1218e8cf6a9e7429604e95c2c7d1c98146ecd7bb`
...
- Release 脚本生成的临时 2026-07-30 记录另存为 `C:\tmp\mediaviewer-2026-07-31-build-script-record.md`。
